...
function calculator(number = 0) {
    ...
    return {
        sum: (...args) => args.reduce((result, el) => result += el, number),
        dif: (...args) => args.reduce((result, el) => result -= el, number),
        div: (...args) => {
            let zerosArr = args.filter( item => item == 0 );

            if (zerosArr.length > 0) {
                throw new Error('division by 0');
            }

            return args.reduce((result, el) => result /= el, number);
        },
        mul: (...args) => args.reduce((result, el) => result *= el, number),
    };
}
